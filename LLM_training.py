# LLM training
from LLM_config import Transformer,PretrainDataset,ModelConfig
...
def train_epoch(epoch):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        if (step + 1) % args.save_interval == 0:
            model.eval()
            # 帶步数的檢查點
            ckp = f'{args.out_dir}/pretrain_{lm_config.dim}_{lm_config.n_layers}_{lm_config.voc_size}_step{step+1}.pth'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start_batch", type=int, default=1750000,help="batch to start training from")#接續訓練需修改此數字

    parser.add_argument("--out_dir", type=str, default="./models/pretrain", help="模型輸出目錄")
    parser.add_argument("--data_path", type=str, default="./dataset/tokenizer_data/seq_monkey_datawhale.jsonl", help="訓練數據路徑")
    ...
    print('Settings Verified:',f'{args.device}' , '-> training start...')
    print('Batch number per epoch:',f'{iter_per_epoch}')

    #接續訓練(if)
    if args.start_batch > -1:
        print('Relay training start...')
        ckp = f'{args.out_dir}/pretrain_{lm_config.dim}_{lm_config.n_layers}_{lm_config.voc_size}_step{args.start_batch}.pth'
        state_dict = torch.load(ckp, map_location=torch.device(args.device))
        if isinstance(model, torch.nn.DataParallel):
        # 移除state_dict鍵中的'module.'前綴
            new_state_dict = {}
    ...
    model.eval()
    ckp = f'{args.out_dir}/pretrain_{lm_config.dim}_{lm_config.n_layers}_{lm_config.voc_size}_final.pth'
    state_dict = model.module.state_dict() if isinstance(model, torch.nn.DataParallel) else model.state_dict()
    torch.save(state_dict, ckp)
    model.train()
